    ...
    <div class="container">
      <h1 class="mb-0">
        <a href="<?= autoUrl("") ?>" class="<?= $clubLogoColour ?>">
          <strong>
            <?= mb_strtoupper(htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))) ?>
          </strong>
        </a>
      </h1>
    </div>
        ...
        if (sizeof($permissions) > 1) { ?>
          ...
          <div class="card">
            ...
            <div class="list-group list-group-flush">
              <?php
              $url = rtrim(nezamy_app('request')->curl, '/');
              $queries = nezamy_app('request')->query;
              $i = 0;
              ...
              <?php foreach ($permissions as $permission) {
                $disabled = $permission == $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'];
              ?>

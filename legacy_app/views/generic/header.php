...
if (nezamy_app()->tenant->getKey('SYSTEM_COLOUR') && getContrastColor(nezamy_app()->tenant->getKey('SYSTEM_COLOUR'))) {
...
<body class="<?= $bg ?> account--body <?php if (isset($pageHead['body-class'])) {
  ...
  <div class="d-print-none">
    ...
    <?php if (nezamy_app()->tenant->getKey('EMERGENCY_MESSAGE_TYPE') != 'NONE' && nezamy_app()->tenant->getKey('EMERGENCY_MESSAGE')) {
    ...
    ?>
      ...
      <div class="py-3 <?php if (nezamy_app()->tenant->getKey('EMERGENCY_MESSAGE_TYPE') == 'DANGER') { ?>bg-danger text-white<?php } ?> <?php if (nezamy_app()->tenant->getKey('EMERGENCY_MESSAGE_TYPE') == 'WARN') { ?>bg-warning text-body<?php } ?> <?php if (nezamy_app()->tenant->getKey('EMERGENCY_MESSAGE_TYPE') == 'SUCCESS') { ?>bg-success text-white<?php } ?>">
        <div class="<?= $container_class ?> emergency-message">
          <?php try { ?>
            <?= $markdown->text(nezamy_app()->tenant->getKey('EMERGENCY_MESSAGE')) ?>
    ...
    <noscript>
      <div class="bg-warning text-dark box-shadow py-3 d-print-none">
        <div class="<?= $container_class ?>">
          ...
          <p class="mb-0">
            If JavaScript is not supported by your browser, <?= nezamy_app()->tenant->getKey('CLUB_NAME') ?>
    ...
    <?php if ($_SESSION['Browser']['Name'] == "Internet Explorer") { ?>
      <div class="bg-warning text-dark py-3 d-print-none">
        <div class="<?= $container_class ?>">
          ...
          <p class="mb-0">
            <?= htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) ?> recommends you <strong><a class="text-dark" href="https://www.firefox.com">install Firefox by Mozilla</a></strong>. Firefox has great protections for your privacy with built in features including tracking protection.
          </p>
        </div>
      </div>
    <?php } ?>

    <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserSimulation'])) { ?>
      <div class="bg-dark text-white py-2 d-print-none">
        <div class="<?= $container_class ?>">
          <p class="mb-0">
            <strong>
              You are in user simulation mode simulating <?=
                                                          $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserSimulation']['SimUserName'] ?>
            </strong>
          </p>
          <p class="mb-0">
    ...
    if (isset($allow_edit) && $allow_edit && (($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] != "Parent" &&
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] != "Coach" && $edit_link != null) || $page_is_mine)) { ?>
    ...
    <div class="membership-header">
        ...
        <div class="<?= $container_class ?>">
          <div class="row justify-content-between align-items-center py-3 mb-0 d-none d-md-flex">
            <div class="col-auto">
              <h1 class="mb-0">
                <a href="<?= htmlspecialchars(autoUrl("")) ?>" class="<?= $clubLogoColour ?> text-decoration-none">
                  ...
                  <?php } else { ?>
                    <?= htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) ?>
                  <?php } ?>
                </a>
              </h1>
            </div>
            <?php if (nezamy_app()->tenant->getKey('CLUB_WEBSITE')) { ?>
              <div class="col-auto">
                <a href="<?= htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_WEBSITE')) ?>" class="btn <?= $clubLinkColour ?> text-decoration-none">Club website</a>
              </div>
            <?php } ?>
          </div>
        </div>
      ...
      <?php if (!isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']) || !user_needs_registration($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'])) { ?>
        <div>
          <div class="<?= $container_class ?>">
            <div class="">
              <div class="">
        ...
        d-print-none justify-content-between px-0" role="navigation">
                  ...
                  <a class="navbar-brand d-lg-none" href="<?= htmlspecialchars(autoUrl("")) ?>">
                    <?php if (nezamy_app()->tenant->getKey('CLUB_SHORT_NAME')) { ?>
                      <?= htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_SHORT_NAME')) ?> Membership
                    <?php } else { ?>
                      <?= htmlspecialchars(nezamy_app()->tenant->getKey('ASA_CLUB_CODE')) ?> Club Membership
                    <?php } ?>
                  </a>
  ...
  <div class="d-none d-print-block">
    <?php
    $addr = json_decode(nezamy_app()->tenant->getKey('CLUB_ADDRESS'));
    $logoPath = null;
    if ($logos = nezamy_app()->tenant->getKey('LOGO_DIR')) {
    ...
    <div class="container">
      <div class="row mb-3">
        <div class="col club-logos">
          ...
          <?php } else { ?>
            <h1 class="primary"><?= htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) ?></h1>
          <?php } ?>
        </div>

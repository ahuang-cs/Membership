...
$do_random_2FA = filter_var(getUserOption($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['2FAUserID'], "IsSpotCheck2FA"), FILTER_VALIDATE_BOOLEAN);
...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorState'])) {
  $errorState = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorState'];
}
...
<div class="container">
  ...
  <div class="row">
    <div class="col-md-8 col-lg-5">
      <?php if (!isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR_GOOGLE']) || $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR_GOOGLE'] !== true) { ?>
        <p class="lead mb-5">
      ...
      <?php if (isset($errorState) && $errorState) { ?>
        <div class="alert alert-danger">
          ...
          Please try again
          <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorStateLSVMessage'])) {
            echo $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorStateLSVMessage'];
            unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorStateLSVMessage']);
          } ?>
        </div>
      <?php } ?>

      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR_RESEND']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR_RESEND']) { ?>
      ...
      <p class="mb-5">
        ...
        <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR_GOOGLE']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR_GOOGLE']) { ?>
      ...
      <p class="small d-none mb-5">
        Unauthorised access to or misuse of this system is prohibited and constitutes an offence under the Computer Misuse Act 1990. If you disclose any information obtained through this system without authority then <?= htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) ?> or Swimming Club Data Systems may take legal action against you.
      </p>
...
unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR_RESEND']);

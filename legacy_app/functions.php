...
function notifySend($to, $subject, $emailMessage, $name = null, $emailaddress = null, $from = null)
{
  ...
  if (!isset($from['Name']) && isset(nezamy_app()->tenant)) {
    $from['Name'] = nezamy_app()->tenant->getKey('CLUB_NAME');
  ...
  if (getenv('SENDGRID_API_KEY') && $emailaddress != null && $name != null) {
    if (isset(nezamy_app()->tenant)) {
      $email->setReplyTo(nezamy_app()->tenant->getKey('CLUB_EMAIL'), nezamy_app()->tenant->getKey('CLUB_NAME'));
    }
...
function getAttendanceByID($link = null, $id, $weeks = "all")
{
  ...
  if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] != 'Parent' || $hideAttendance) {
  ...
  if ($status == "paid_out") {
    ...
    try {
      ...
      if ($getCount->fetchColumn() == 0) {
        ...
        $message .= '<p>Kind regards,<br>The ' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . ' Team</p>';
    ...
    try {
      ...
      <p>Kind regards,<br>The ' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . ' Team</p>';
      $query = $db->prepare("INSERT INTO notify (UserID, Status, Subject, Message, ForceSend, EmailType) VALUES (?, ?, ?, ?, ?, ?)");
      $query->execute([$details['UserID'], 'Queued', $subject, $message, 1, 'Payments']);
...
function curl($url)
{
  ...
  curl_setopt($ch, CURLOPT_HTTPHEADER, array('Origin: ' . nezamy_app('request')->hostname));
...
{
  $db = nezamy_app()->db;
  if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'])) {
  ...
  if ($email != isSubscribed($user, $list)) {
    $email_update = true;
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['OptionsUpdate'] = true;
  }
...
/*
if ( (empty($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LoggedIn']) || empty($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Username'])) && ($preventLoginRedirect != true)) {
  // Allow access to main page
  header("Location: " . autoUrl("login.php"));
}
elseif (((!empty($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LoggedIn'])) || (!empty($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Username']))) && ($preventLoginRedirect == true)) {
...
{
  if ($absolute) {
    return getenv('STRIPE') && nezamy_app()->tenant->getBooleanKey('USE_STRIPE_DIRECT_DEBIT');
  }
  return getenv('STRIPE') && nezamy_app()->tenant->getBooleanKey('ALLOW_STRIPE_DIRECT_DEBIT_SET_UP') || nezamy_app()->tenant->getBooleanKey('USE_STRIPE_DIRECT_DEBIT');
}

function stripeSetUpDirectDebit()
{
  return getenv('STRIPE') && nezamy_app()->tenant->getBooleanKey('ALLOW_STRIPE_DIRECT_DEBIT_SET_UP');
}
...
function handleCompletedGalaPayments($paymentIntent, $onSession = false)
{
  ...
  */
  ...
  if ($onSession && $intent->status == 'succeeded') {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['CompletedEntryInfo'] = $databaseId;
    unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentIntent']);
    unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PaidEntries']);
    unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentMethodID']);
    unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AddNewCard']);

    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentSuccess'] = true;
  ...
  if ($userId == null) {
    $userId = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'];
  }
  ...
  if (isset($newMethod) && $newMethod) {
    ...
    try {
      ...
      if ($onSession) {
        $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PayCardError'] = true;
        $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PayCardErrorMessage'] = $err;
        header("Location: " . autoUrl("galas/pay-for-entries/checkout"));
      } else {
        reportError($e);
      }
  ...
  if ($intent->status == 'succeeded') {
    ...
    try {
      ...
      if (nezamy_app()->tenant->isCLS()) {
        ...
        $sendingEmail = mb_strtolower(trim(nezamy_app()->tenant->getKey('ASA_CLUB_CODE'))) . "-payments@" . getenv('EMAIL_DOMAIN');
      }
      notifySend(null, 'Payment Receipt', $message, $name, $email, [
        "Email" => $sendingEmail,
        "Name" => nezamy_app()->tenant->getKey('CLUB_NAME'),
        "Unsub" => [
          "Allowed" => false,
          "User" => $userId,
          "List" =>  "Payments"

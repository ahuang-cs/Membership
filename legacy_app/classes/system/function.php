...
function pre($arr, $them = 1)
{
  ...
  <pre style="direction: ltr;background:<?= $background; ?>;color:<?= $color; ?>;
    ...
    font-family: Monaco,Consolas, 'Lucida Console',monospace;font-size: 16px;padding: 20px;">
    <?php
    print_r($arr);
    echo "</pre>";
  }
  ...
  //=======[ get domain url ]=========================
  function nezamy_url($path = null)
  {
    return URL . rtrim(trim($path), '/') . '/';
  }

  //=======[ get route url by name  ]=================
  function nezamy_route($name, array $args = [])
  {
    return nezamy_url(nezamy_app('route')->get_route($name, $args));
  }

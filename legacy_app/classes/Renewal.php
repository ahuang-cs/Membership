...
class Renewal
{
  ...
  private function revalidateMembers()
  {
    ...
    $getMember = nezamy_app()->db->prepare("SELECT MForename fn, MSurname sn, UserID FROM members WHERE MemberID = ?");
  ...
  {
    try {
      $update = nezamy_app()->db->prepare("UPDATE renewalData SET Document = JSON_REPLACE(Document, REPLACE(JSON_UNQUOTE(JSON_SEARCH(Document, 'one', ?, '', '$.progress')), '.object', '.completed'), ?) WHERE ID = ?");
      $update->execute([
        $objectType,
        $state,
        $this->id,

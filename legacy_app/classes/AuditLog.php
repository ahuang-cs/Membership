...
class AuditLog
{
  ...
  public static function new(string $event, string $description, int $user = null)
  {
    ...
    if (!$user) {
      ...
      $user = nezamy_app()->user->getId();
      if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserSimulation']['RealUser'])) {
        $user = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserSimulation']['RealUser'];
      }
    }

    $time = new DateTime('now', new DateTimeZone('UTC'));

    $add = nezamy_app()->db->prepare("INSERT INTO `auditLogging` (`ID`, `User`, `Time`, `Event`, `Description`) VALUES (?, ?, ?, ?, ?)");
    $add->execute([

...
class Tenant
{
  ...
  {
    $count = nezamy_app()->db->prepare("SELECT COUNT(*) FROM gcCredentials WHERE OrganisationId = ? OR Tenant = ?");
    $count->execute([
      $orgId,
      $this->id
    ]);
    ...
    $insert = nezamy_app()->db->prepare("INSERT INTO gcCredentials (OrganisationId, AccessToken, Tenant) VALUES (?, ?, ?)");
    $insert->execute([
      $orgId,
      $accessToken,
      $this->id

...
class User extends Person
{
  ...
  public function revalidate()
  {
    ...
    if ($row) {
      ...
      } else if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel']) && in_array($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'], $this->permissions)) {
        $this->accessLevel = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'];
      ...
      if ($this->setSession) {
        ...
        $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Forename'] = $this->forename;
        $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Surname'] = $this->surname;
        $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailAddress'] = $this->emailAddress;
        $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] = $this->accessLevel;
      }
    } else {
      throw new Exception();
    }
  ...
  {
    $ec = new EmergencyContacts(nezamy_app()->db);
    $ec->byParent($this->id);
  ...
  {
    if (!nezamy_app()->tenant->getStripeAccount() || !getenv('STRIPE')) {
      return null;
    }

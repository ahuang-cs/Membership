...
class Login
{
  ...
  public function login()
  {
    ...
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailAddress'] = $details['EmailAddress'];
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Forename'] = $details['Forename'];
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Surname'] = $details['Surname'];
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'] = $details['UserID'];
    ...
    try {
      ...
      $record = $reader->city(nezamy_app('request')->ip());
      $city = '';
      if ($record->city->name != "") {
    ...
    if (!$this->reLogin) {
      ...
      setcookie(COOKIE_PREFIX . 'TENANT-' . nezamy_app()->tenant->getId() . '-' . 'AutoLogin', $hash, time() + 60 * 60 * 24 * 120, $cookiePath, nezamy_app('request')->hostname, $secure, false);
    }
    ...
    $login_before_data = [
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'],
      nezamy_app('request')->ip(),
      ucwords(nezamy_app('request')->browser()),
      ucwords(nezamy_app('request')->platform())
    ];
    ...
    if ($now > $dec1 && isset($_SESSION['Browser']['OSName']) && $_SESSION['Browser']['OSName'] == "Android" && isset($_SESSION['Browser']['OSVersion']) && (float) $_SESSION['Browser']['OSVersion'] <= 7.1 && !$this->noUserWarning && !$this->reLogin) {
      ...
      $message .= '<p><small>SCDS provides the membership system to ' . htmlspecialchars(nezamy_app()->tenant->getName()) . '.</small></p>';
      notifySend(null, $subject, $message, $details['Forename'] . ' ' . $details['Surname'], $details['EmailAddress'], [
        'Name' => 'SCDS and ' . nezamy_app()->tenant->getName(),
      ]);
    }

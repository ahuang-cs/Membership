...
class CreateMail
{
  ...
  public function getFormattedHtml()
  {
    ...
    if (isset(nezamy_app()->tenant) && nezamy_app()->tenant->isCLS()) {
      ...
      $head .= "<img src=\"" . autoUrl($logos . 'logo-150.png') . "\" srcset=\"" .
        ...
        autoUrl($logos . 'logo-150@3x.png') . " 3x\" style=\"max-width:100%;max-height:150px;\" alt=\"" . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . " Logo\">";
    ...
    if (isset(nezamy_app()->tenant)) {
      $foot .= "
      <p class=\"small\" align=\"center\"><strong>" . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . "</strong><br>";
    ...
    if (isset(nezamy_app()->tenant)) {
      $foot .= "
    <p class=\"small\" align=\"center\">This email was sent via the " . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . " Membership System.</p>";
    ...
    <p class=\"small\" align=\"center\">To control your email options, go to <a href=\"" .
      ...
      $foot .= "
    <p class=\"small\" align=\"center\">&copy; " . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . " " . date("Y") . ", Design &copy; SCDS</p>";
    }
    $foot .= "
      </div>
      </table>
  ...
  public function getFormattedPlain()
  {
    ...
    if (isset(nezamy_app()->tenant)) {
      $foot = "\r\n\n\n " . nezamy_app()->tenant->getKey('CLUB_NAME') . "\r\n\r\n";
      $foot .= nezamy_app()->tenant->getKey('CLUB_NAME') . "\r\n";
      $addr = json_decode(nezamy_app()->tenant->getKey('CLUB_ADDRESS'));
      if ($addr) {
        for ($i = 0; $i < sizeof($addr); $i++) {
          $foot .= $addr[$i] . "\r\n";
      ...
      $foot .= "\r\nThis email was sent automatically by the " . nezamy_app()->tenant->getKey('CLUB_NAME') . " Membership System.\r\n\r\n";
      $foot .= "Have questions? Contact us at " . nezamy_app()->tenant->getKey('CLUB_EMAIL') . ".\r\n\r\n";
      $foot .= "To control your email options go to My Account at " . autoUrl("myaccount") . ".\r\n\r\n";
      if ($this->allowUnsubscribe) {
        $foot .= "Unsubscribe at -unsub_link-\r\n\r\n";
      }
      $foot .= "Content copyright " . date("Y") . " " . nezamy_app()->tenant->getKey('CLUB_NAME') . ", Design copyright SCDS";
    } else {

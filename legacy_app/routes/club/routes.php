...
$currentUser = null;
if (empty($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LoggedIn']) && isset($_COOKIE[COOKIE_PREFIX . 'TENANT-' . nezamy_app()->tenant->getId() . '-' . 'AutoLogin']) && $_COOKIE[COOKIE_PREFIX . 'TENANT-' . nezamy_app()->tenant->getId() . '-' . 'AutoLogin'] != "") {
  ...
  if ($row != null) {
    ...
    $hash = hash('sha512', time() . $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'] . random_bytes(64));

    $sql = "UPDATE `userLogins` SET `Hash` = ? WHERE `Hash` = ?";
    try {
      $query = $db->prepare($sql);
      $query->execute([$hash, $_COOKIE['TENANT-' . nezamy_app()->tenant->getId() . '-' . 'AutoLogin']]);
    } catch (PDOException $e) {
      halt(500);
    }
    ...
    $cookiePath = '/' . nezamy_app()->tenant->getCodeId();
    setcookie('TENANT-' . nezamy_app()->tenant->getId() . '-' . "AutoLogin", $hash, $expiry_time, $cookiePath, nezamy_app('request')->hostname, $secure, false);
  }
} else if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'])) {
  $currentUser = new User($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], true);
}
nezamy_app()->user = $currentUser;

if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LoggedIn']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LoggedIn'] && !isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['DisableTrackers'])) {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['DisableTrackers'] = filter_var(getUserOption($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], "DisableTrackers"), FILTER_VALIDATE_BOOLEAN);
}
...
$this->get('/auth/cookie/redirect', function () {
  ...
  setcookie('TENANT-' . nezamy_app()->tenant->getId() . '-' . "SeenAccount", true, 0, "/", nezamy_app('request')->hostname, true, false);
...
$this->post('/check-login.json', function () {
  ...
  echo json_encode(['signed_in' => isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LoggedIn']) && bool($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LoggedIn'])]);
});
...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR']) {
  ...
  $this->get(['/', '/*'], function () {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR'] = true;
...
if (empty($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LoggedIn'])) {
  ...
  //   http_response_code(303);
  //   header("Location: " . autoUrl("login?club=" . mb_strtolower(nezamy_app()->tenant->getCodeId()), false));
  // });
  ...
  */
  ...
  // $this->any(['/', '/*'], function () {
  //   http_response_code(303);
  //   header("Location: " . autoUrl("login?club=" . mb_strtolower(nezamy_app()->tenant->getCodeId() . '&target=' . urlencode(nezamy_app('request')->path)), false));
  // });

  $this->any(['/', '/*'], function () {
    http_response_code(303);
    header("Location: " . autoUrl('login?target=' . urlencode(nezamy_app('request')->path)));
  });

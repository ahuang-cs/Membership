...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegUser']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegUser']) {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegUser'] = null;
  unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegUser']);
}
...
if ($status && $info) {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegUser'] = $info['UserID'];
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegExisting'] = true;
  ...
  // USER DOES NOT EXIST
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegUserEmail'] = $email;
  header("Location: " . autoUrl("assisted-registration/start"));
} else if (!$status) {
  // INVALID EMAIL ADDRESS
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegEmailError'] = 'INV-EMAIL';
  header("Location: " . autoUrl("assisted-registration#get-started"));
} else {
  // NOT A PARENT ACCOUNT
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegEmailError'] = 'NOT-PARENT';
  header("Location: " . autoUrl("assisted-registration#get-started"));
}

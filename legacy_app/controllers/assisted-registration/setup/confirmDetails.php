...
$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegStage'] = 2;
...
$getUser->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGuestUser']]);
...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsError']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsError']) {
  if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsPostData']['emailAuthorise']) {
    $email = " checked ";
  }
  if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsPostData']['smsAuthorise']) {
    $sms = " checked ";
  }
}
...
<div class="container">
  <div class="row">
    <div class="col-md-8">
      ...
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsError']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsError']) { ?>
        <div class="alert alert-danger">
          <p><strong>There was a problem</strong></p>
          <?= $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsMessage'] ?>
        </div>
      <?php } ?>
      ...
      <form method="post" class="needs-validation" novalidate>
          ...
          <div class="col-md-8 col-lg-6">
            ...
            <div class="form-group">
              <div class="custom-control custom-checkbox">
                ...
                <small class="d-block"><?= htmlspecialchars(nezamy_app()->tenant->getName()) ?> may not offer this service</small>
              </div>
            </div>
        ...
        <div class="cell">
          ...
          <?php } else { ?>
            ...
            <p class="mb-0">
              Contact a member of your committee if you have any questions or email <?php if (nezamy_app()->tenant->isCLS()) { ?><a href="mailto:support@chesterlestreetasc.co.uk">support@chesterlestreetasc.co.uk</a><?php } else { ?><a href="mailto:<?= htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_EMAIL')) ?>"><?= htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_EMAIL')) ?></a><?php } ?>.
            </p>
          <?php } ?>
        </div>
...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsMessage'])) {
  unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsMessage']);
}
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsError'])) {
  unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsError']);
}
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsPostData'])) {
  unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsPostData']);
}

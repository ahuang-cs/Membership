...
$countSwimmers->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGuestUser']]);
$rr = false;
...
if (mb_strlen($user['Mobile']) == 0) {
  ...
  try {
    ...
    $updateMobile->execute([
      $mobile,
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGuestUser']
    ]);
...
if ($status) {
  ...
  try {
    ...
    $update->bindValue(5, $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGuestUser'], PDO::PARAM_INT);
    $update->execute();

    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegStage'] = 3;
    header("Location: " . autoUrl("assisted-registration/go-to-onboarding"));
  } catch (Exception $e) {
    reportError($e);
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsError'] = true;
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsPostData'] = $_POST;
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegGetDetailsMessage'] = $statusMessage;

...
$user = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegUser'];
...
try {
  ...
  $user->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegUser']]);
  ...
  while ($swimmer = $swimmers->fetch(PDO::FETCH_ASSOC)) {
    if ($_POST['member-' . $swimmer['id']]) {
      ...
      $setParent->execute([
        $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegUser'],
...
if ($success) {
  // Go to next page
  $subject = "Complete your registration at " . nezamy_app()->tenant->getKey('CLUB_NAME');
  $message = "<p>Hello " . htmlspecialchars($user['first']) . ", </p>";
  if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegExisting']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegExisting']) {
    $message .= "<p>We've added a new member to your " . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . " account. We now need you to <a href=\"" . autoUrl("") . "\">sign in and provide some information</a>.</p>";
    unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegExisting']);
  } else {
    ...
    $message .= "<p>To continue, <a href=\"" . autoUrl("assisted-registration/" . $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegUser'] . "/" . $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegPass']) . "\">please follow this link</a></p>";
    if ($tenant->getBooleanKey('REQUIRE_FULL_REGISTRATION')) {
      $message .= "<p>As part of the registration process, we'll ask you to set a password, let us know your communication preferences and fill in important information about you and/or your members. At the end, we'll set up a direct debit so that payments to " . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . " are taken automatically.</p>";
      $message .= "<p>You'll also be given the opportunity to set up a direct debit.</p>";
    }
  }
  $message .= '<p>Please note that ' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . ' may not provide all services included in the membership software.</p>';

  notifySend(null, $subject, $message, $user['first'] . ' ' . $user['last'], $user['email']);

  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegComplete'] = true;
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegName'] = $user['first'] . ' ' . $user['last'];

...
<div class="container">
  <div class="row">
    <div class="col-md-8">
      ...
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegEmailError'])) { ?>
      <div class="alert alert-danger">
        <?php if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegEmailError'] == 'INV-EMAIL') { ?>
        <strong>The email address provided was not valid</strong>
        <?php } else { ?>
        <strong>A user already exists but is not a parent account</strong>
        <?php } unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AssRegEmailError']); ?>
      </div>
      <?php } ?>

...
try {
  ...
  if (isset($_POST['submission'])) {
    ...
    $getSubmission = $db->prepare("SELECT `Member`, `DateTime` FROM covidRiskAwareness INNER JOIN members ON covidRiskAwareness.Member = members.MemberID WHERE covidRiskAwareness.ID = ? AND members.Tenant = ?");
    ...
    if (!$rep && !$user->hasPermission('Admin') && !$user->hasPermission('Coach') && !$user->hasPermission('Galas')) {
      if ($member['UserID'] != $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']) {
        throw new Exception('Not found');
      }
    }
    ...
    if ($memberUser) {
      $subject = 'We need a new COVID-19 Risk Awareness Form for ' . $member['MForename'];
      ...
      $message .= '<p>We have voided ' . htmlspecialchars($member['MForename'] . ' ' . $member['MSurname']) . '\'s COVID-19 Risk Awareness Form which was submitted at ' . htmlspecialchars($subTime->format('H:i, j F Y')) . '.</p>';

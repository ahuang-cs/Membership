...
$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['qr'][0]['text'] = autoUrl("form-agreement/m/" . urlencode('CodeOfConduct') . '/' . urlencode(date("Y-m-d")) . "/" . urlencode($id) . '/' . urlencode("Squad: " . $email_info['SquadName']));
$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['qr'][0]['size'] = 600;
$qrFile = true;
...
<html>
  ...
  <body>
    ...
    <!--<p>&copy; <?=htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))?> <?=date("Y")?></p>-->
    ...
    <div class="prevent-page-break">
      <div class="cell">
        ...
        <p>
          I, <?=htmlspecialchars($email_info['MForename'] . " " . $email_info['MSurname'])?> agree to the Code of Conduct for <?=htmlspecialchars($email_info['SquadName'])?> Squad as outlined above as required by the Terms and Conditions of Membership of <?=htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))?>.
        </p>

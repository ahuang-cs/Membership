...
if ($_POST['squadDeleteDanger'] ==  $squadDeleteKey) {
  ...
  try {
    ...
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateDatabaseError'] = true;
  }

  if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateDatabaseError']) {
    header("Location: " . autoUrl("squads/" . $id . "/edit"));
  } else {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['DeleteSuccess'] = true;

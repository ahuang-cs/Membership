		...
		if ($sendEmail) {
			...
			if ($email_info) {
				...
				try {
					...
					$email->setFrom("noreply@" . getenv('EMAIL_DOMAIN'), nezamy_app()->tenant->getKey('CLUB_NAME'));
					$email->setFrom("noreply@" . getenv('EMAIL_DOMAIN'), nezamy_app()->tenant->getKey('CLUB_NAME'));
					if (nezamy_app()->tenant->getKey('CLUB_EMAIL')) {
						$email->setReplyTo(nezamy_app()->tenant->getKey('CLUB_EMAIL'), nezamy_app()->tenant->getKey('CLUB_NAME') . ' Team');
					}

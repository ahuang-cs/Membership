...
<div class="container">
  ...
  <div class="row">
    <div class="col-lg-8 col-md-10">
      ...
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivSuccess'])) {
        if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivSuccess']) {?>
      ...
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifySuccess'])) { ?>
      <div class="alert alert-success">
        We've successfully queued your email. <?=
    		$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifySuccess']['Count'] ?> people will receive your
        message<?php if (!$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifySuccess']['Force']) { ?> if they have
        opted in to recieving emails from us<?php } ?>.
      </div>
      <?php }
      unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifySuccess']); ?>
      ...
      <p class="small">
        Provided by Swimming Club Data Systems to <?=htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))?>.
      </p>
    </div>
...
unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivSuccess']);

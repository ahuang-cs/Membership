...
try {
  ...
  $fromName = nezamy_app()->tenant->getKey('CLUB_NAME');
  ...
  if (!($replyAddress && isset($_POST['ReplyToMe']) && bool($_POST['ReplyToMe']))) {
    $replyAddress = nezamy_app()->tenant->getKey('CLUB_EMAIL');
    $replyName = nezamy_app()->tenant->getKey('CLUB_NAME');
  }
  ...
  if (isset($swimmer) && isset($_POST['coach-send']) && bool($_POST['coach-send'])) {
    ...
    $bccEmails[nezamy_app()->user->getEmail()] = nezamy_app()->user->getName();
  ...
  if ($response->statusCode() == "202") {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivSuccess'] = true;
  } else {
  ...
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivSuccess'] = false;
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivPostContent'] = $_POST;
  reportError($e);
} finally {

  if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivSuccess']) && !$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivSuccess']) {
    // Return to composer

...
$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyPostData'] = $_POST;
...
try {
  if (sizeof($_POST) == 0) {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TooLargeError'] = true;
    throw new Exception('Filesize TooLargeError');
  }

  if (!SCDS\FormIdempotency::verify()) {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['FormError'] = true;
  ...
  $replyAddress = getUserOption($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], 'NotifyReplyAddress');
    ...
    if (is_uploaded_file($_FILES['file-upload']['tmp_name'][$i])) {
      ...
      if (bool($_FILES['file-upload']['error'][$i])) {
        ...
        if ($_FILES['file-upload']['error'][$i] == 2) {
          ...
          $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UploadError'] = true;
        }
        throw new Exception();
  ...
  if ($collectiveSize > 10485760) {
    ...
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['CollectiveSizeTooLargeError'] = true;
    throw new Exception();
  }
  ...
  if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] != "Admin" && !($replyAddress && isset($_POST['ReplyToMe']) && bool($_POST['ReplyToMe']))) {
    $name = getUserName($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']);
  ...
  $force = 0;
  $sender = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'];
  if (isset($_POST['force']) && bool($_POST['force']) && ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == "Admin" || $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == "Galas")) {
    $force = 1;
  }
  ...
  if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] != 'Parent') {
    ...
    $lists->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
  }
  ...
  if ($galaQuery && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] != 'Parent') {
  ...
  if ($_POST['from'] == "current-user") {
    ...
    if (!nezamy_app()->tenant->isCLS()) {
      $fromEmail .= '.' . urlencode(strtolower(str_replace(' ', '', nezamy_app()->tenant->getKey("ASA_CLUB_CODE"))));
    }
  ...
  if ($replyAddress && isset($_POST['ReplyToMe']) && bool($_POST['ReplyToMe'])) {
    $recipientGroups["ReplyToMe"] = [
      "Email" => $replyAddress,
      "Name" => $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Forename'] . ' ' . $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Surname'],
    ];
  }
  ...
  // if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] != "Admin" && $force == 1) {
  ...
  //   $sendToTeam->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], null, "GDPR Compliance: " . $subject, $gdpr_question, $sender, $force]);

  //   $intro = '
  //   <p>We\'re sending you this email because you\'re an administrator at ' . nezamy_app()->tenant->getKey('CLUB_NAME') . '.</p>
  //   <p>' . getUserName($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']) . ' has force sent the following email, overriding parent subscription options. We send these updates about emails which have been force sent in order to ensure compliance with GDPR rules.</p>
  ...
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifySuccess'] = [
    "Count" => $count,
    "Force" => $force
  ];
  ...
  while ($currentMessage = $getPendingGroupMail->fetch(PDO::FETCH_ASSOC)) {
    ...
    $from = new \SendGrid\Mail\From("noreply@" . getenv('EMAIL_DOMAIN'), nezamy_app()->tenant->getKey('CLUB_NAME'));
    ...
    if ($jsonData->ReplyToMe->Email != null && $jsonData->ReplyToMe->Name != null) {
      try {
        ...
        $email->setReplyTo(nezamy_app()->tenant->getKey('CLUB_EMAIL'), nezamy_app()->tenant->getKey('CLUB_NAME') . ' Enquiries');
      }
    } else {
      $email->setReplyTo(nezamy_app()->tenant->getKey('CLUB_EMAIL'), nezamy_app()->tenant->getKey('CLUB_NAME') . ' Enquiries');
    }

    $email->addHeader("List-ID", getenv('CLUB NAME') . " Notify <" . mb_strtolower(nezamy_app()->tenant->getKey('ASA_CLUB_CODE')) . ".notify@" . getenv('EMAIL_DOMAIN') . ">");
  ...
  if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyPostData'])) {
    unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyPostData']);
  }

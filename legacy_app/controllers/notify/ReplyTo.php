...
<div class="container">
  ...
  <div class="row">
    <div class="col-lg-8">
      ...
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SetReplySuccess']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SetReplySuccess']) { ?>
      ...
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SetReplySuccess']); } ?>

      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SetReplyFalse']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SetReplyFalse']) { ?>
        <div class="alert alert-danger">
          <p class="mb-0"><strong>Your email address was not valid</strong></p>
          <p class="mb-0">Please try again.</p>
        </div>
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SetReplyFalse']); } ?>

      <form method="post">
        <div class="form-group">
          <label for="reply">Reply-To email address</label>
          <input type="email" class="form-control" id="reply" name="reply" placeholder="Enter email" value="<?=htmlspecialchars(getUserOption($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], 'NotifyReplyAddress'))?>">
        </div>

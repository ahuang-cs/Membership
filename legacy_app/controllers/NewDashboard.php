...
$username = htmlspecialchars(explode(" ", getUserName($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']))[0]);
...
$sql = "SELECT SessionID, SessionName, VenueName, StartTime, EndTime FROM (`sessions` INNER JOIN sessionsVenues ON sessions.VenueID = sessionsVenues.VenueID) WHERE SessionDay = :day_ow AND StartTime <= :timenow AND (EndTime > :timenow OR EndTime > :time30) AND DisplayFrom <= CURDATE() AND DisplayUntil >= CURDATE() AND `sessions`.`Tenant` = :tenant ORDER BY StartTime ASC, EndTime ASC";
...
$query->execute(['day_ow' => $day, 'timenow' => $time, 'time30' => $time30, 'tenant' => $tenant->getId()]);
...
<div class="front-page mb-n3">
	<div class="container">
		...
		<div class="mb-4">
			...
			<div class="news-grid">
				...
				<a href="<?= autoUrl('covid/risk-awareness') ?>">
					<span class="mb-3">
						<span class="title mb-0">
							<?php if (mb_strtoupper(nezamy_app()->tenant->getKey('ASA_CLUB_CODE')) == 'UOSZ') { ?><?= htmlspecialchars(UOS_RETURN_FORM_NAME) ?><?php } else { ?>Risk Awareness Declaration<?php } ?>
						</span>
						<span>
		...
		<?php if (nezamy_app()->tenant->getKey('ASA_DISTRICT') == 'E' && $asa_ne != null) { ?>
			<div class="mb-4">

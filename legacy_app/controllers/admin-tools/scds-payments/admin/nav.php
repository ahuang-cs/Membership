<nav class="navbar navbar-expand-lg navbar-dark bg-dark sticky-top rounded-bottom">
  ...
  <div class="collapse navbar-collapse" id="admin-payments-nav">
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item">
        <a class="nav-link" href="<?= htmlspecialchars(autoUrl('payments-admin/exit')) ?>" title="Return to the <?= htmlspecialchars(nezamy_app()->adminCurrentTenant->getName()) ?> Membership System">Exit</a>
      </li>
    </ul>
    <span class="navbar-text small">
    <?= htmlspecialchars(nezamy_app()->adminCurrentUser->getName()) ?>, <?= htmlspecialchars(nezamy_app()->adminCurrentTenant->getName()) ?>
    </span>
  </div>
</nav>
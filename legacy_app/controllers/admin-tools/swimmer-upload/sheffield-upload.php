...
<div class="container">
  ...
  <div class="row">
    <div class="col-lg-8">
      ...
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['FailedSwimmers'])) { ?>
        <div class="alert alert-warning">
          <p>We've uploaded most of your swimmers but the following could not be uploaded as their squad could not be found;</p>
          <ul class="mb-0">
            <?php foreach ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['FailedSwimmers'] as $s) { ?>
              <li><?= htmlspecialchars($s) ?></li>
            <?php } ?>
          </ul>
        </div>
      <?php
        unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['FailedSwimmers']);
      } ?>

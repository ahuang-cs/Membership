...
if (is_uploaded_file($_FILES['file-upload']['tmp_name'])) {
  ...
  if (bool($_FILES['file-upload']['error'])) {
    ...
    try {
        ...
        if (isset($row[1]) && ((int) $row[1]) != 0 && v::email()->validate(mb_strtolower(trim($row[7])))) {
          ...
          if (!$member) {
            ...
            if ($user) {
              ...
              $content .= '<p>Please note that ' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . ' may not provide all services included in the membership software.</p>';
      ...
      $db->commit();
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UploadSuccess'] = true;
    } catch (Exception $e) {
      $db->rollBack();
      reportError($e);
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UploadError'] = true;
    ...
    if (sizeof($failedSwimmers) > 0) {
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['FailedSwimmers'] = $failedSwimmers;
    }
  }
} else {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UploadError'] = true;

	...
	if ((string) $_POST[$entry['EntryID'] . '-amount'] != "") {
		...
		if ($amount > 0 && $amount <= 15000 && !$hasNoDD) {
			...
			try {
				...
				$message = '<p>We\'ve charged <strong>&pound;' . $amountString . '</strong> to your account for ' . htmlspecialchars($entry['MForename']) .  '\'s entry into ' . htmlspecialchars($gala['name']) . '.</p><p>You will be able to see this charge in your pending charges and from the first day of next month, on your bill statement. You\'ll be charged for this as part of your next direct debit payment to ' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . '.</p>';
				...
				// A problem occured
				$db->rollBack();
				$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ChargeUsersFailure'] = true;
			}
		} else if ($amount > 15000) {
			$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['OverhighChargeAmount'][$entry['EntryID']] = true;
		}
	}
}

if (!isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ChargeUsersFailure'])) {
	$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ChargeUsersSuccess'] = true;
}

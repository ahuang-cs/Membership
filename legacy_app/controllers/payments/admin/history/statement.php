...
<div class="container">
  ...
  <?php } ?>
	...
	<div class="">
    <span class="d-none d-print-block h1"><?=htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))?> Payments</span>
    <?php if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == "Parent") { ?>
    ...
    <?php if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == "Admin" && ($payment_info['Status'] == 'customer_approval_denied' || $payment_info['Status'] == 'failed')) {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Token' . $id] = hash('sha256', random_int(0, 999999));
    $url = autoUrl("payments/statements/" . $id . "/mark-paid/" . $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Token' . $id]);
    ?>
    <p>
    ...
    <div class="row">
      ...
      -->
        ...
        <p>
          Payments to <?=htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))?> are covered by the Direct Debit Guarantee.
        </p>
      </div>
    </div>
	</div>

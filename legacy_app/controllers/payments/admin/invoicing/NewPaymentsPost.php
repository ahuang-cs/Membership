...
try {
  ...
  if ($amount > 0) {
    ...
    $message .= '<p>We\'ve manually added a ' . $typeString . ' of <strong>&pound;' . $amountString . '</strong> to your next ' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . ' direct debit payment for <strong>' . htmlspecialchars($description) .  '</strong>.</p><p>You will be able to see this charge in your pending charges and from the first day of next month, on your bill statement. You\'ll be charged for this as part of your next direct debit payment to ' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . '.</p>';

    $message .= '<p>Kind Regards, <br>The ' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . ' Payments Team</p>';
  ...
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NewPaymentSuccessMessage'] = 'We\'ve added the ' . $typeString . ' to ' . $user['Forename'] . '\'s account to pay in their next bill.';
} catch (PDOException $e) {
  reportError($e);
  $db->rollBack();

  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NewPaymentErrorMessage'] = 'A database error occurred. Please try again. If the error occurs again, please try again later.';
} catch (Exception $e) {
  reportError($e);
  $db->rollBack();

  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NewPaymentErrorMessage'] = $e->getMessage();
}

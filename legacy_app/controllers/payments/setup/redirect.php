...
$selectSchedule->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
...
if ($selectSchedule->fetchColumn() == 0) {
  ...
  try {
    ...
    $redirectFlow = $client->redirectFlows()->complete(
      ...
      ["params" => ["session_token" => $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Token']]]
    );
    ...
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GC-Setup-Status'] = 'success';
    ...
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GC-Setup-Status'] = $e->getType();
    $db->rollBack();
    header("Location: " . autoUrl($url_path . "/setup/4"));

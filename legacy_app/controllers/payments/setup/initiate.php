...
try {
  ...
  $getPaySchdeule->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
...
if ($scheduleExists == null) {
  ...
  $getDetails->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
  $row = $getDetails->fetch(PDO::FETCH_ASSOC);

  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Token'] = hash('sha256', $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'] . "-" . rand(1000,9999));
  ...
  try {
    $redirectFlow = $client->redirectFlows()->create([
      "params" => [
        ...
        "session_token" => $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Token'],
    ...
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GC_REDIRECTFLOW_ID'] = $redirectFlow->id;

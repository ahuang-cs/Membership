...
$getCount = null;
if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Admin' && isset($_GET['users']) && $_GET['users'] == 'all') {
  ...
  $getCount->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
}
...
if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Admin' && isset($_GET['users']) && $_GET['users'] == 'all') {
  ...
  $payments->bindValue(':user', $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], PDO::PARAM_INT);
  $payments->bindValue(':offset', $start, PDO::PARAM_INT);
  $payments->bindValue(':num', 10, PDO::PARAM_INT);
  $payments->execute();
}
          ...
          <a href="<?= htmlspecialchars(autoUrl("payments/card-transactions/" . $pm['ID'])) ?>" class="list-group-item list-group-item-action text-dark">
            ...
            <?php if (isset($pm['Brand'])) { ?>
              <div class="row align-items-center mb-2">
                <div class="col-auto">
                  <img src="<?= autoUrl("public/img/stripe/" . $pm['Brand'] . ".svg") ?>" class="accepted-network-logo"> <span class="sr-only"><?= htmlspecialchars(getCardBrand($pm['Brand'])) ?></span>
                </div>
                <div class="col-auto">
                  <h2 class="my-0">

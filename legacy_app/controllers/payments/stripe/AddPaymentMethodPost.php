...
$setupIntent = null;
if (!isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['StripeSetupIntentId'])) {
  halt(404);
} else {
  try {
    $setupIntent = \Stripe\SetupIntent::retrieve(
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['StripeSetupIntentId'],
      [
        'stripe_account' => $tenant->getStripeAccount()
      ]
    );
  } catch (Exception $e) {
    unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['StripeSetupIntentId']);
...
$getUserEmail->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
$user = $getUserEmail->fetch(PDO::FETCH_ASSOC);

$checkIfCustomer = $db->prepare("SELECT COUNT(*) FROM stripeCustomers WHERE User = ?");
$checkIfCustomer->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
...
if ($checkIfCustomer->fetchColumn() == 0) {
  ...
  $customer = \Stripe\Customer::create([
    ...
    "description" => "Customer for " . $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'] . ' (' . $user['EmailAddress'] . ')',
  ...
  $getCustID->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
...
try {
  ...
  if ($getCardCount->fetchColumn() > 0) {
    ...
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PayCardErrorMessage'] = 'This card is already connected to your account';
    header("Location: " . autoUrl("payments/cards/add"));
    return;
  }
  ...
  unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['StripeSetupIntentId']);
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PayCardSetupSuccess'] = true;
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PayCardSetupSuccessBrand'] = getCardBrand($brand);

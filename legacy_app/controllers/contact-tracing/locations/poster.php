...
$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['qr'][0]['text'] = autoUrl("contact-tracing/check-in/" . $id);
$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['qr'][0]['size'] = 1024;
$qrFile = true;
...
<body style="padding-bottom: 0; margin-bottom: 0;">
  ...
  if ($logos = nezamy_app()->tenant->getKey('LOGO_DIR')) {
    $logoPath = nezamy_app()->tenant->getFilePath() . 'public/' . mb_substr($logos, 8) . 'logo-1024.png';
  }
  ...
  <div style="text-align: center;">
    ...
    <p style="margin: none; padding: none; line-height: 16pt;">
      Go to <strong><?= htmlspecialchars(nezamy_app('request')->hostname) ?></strong>, find <strong><?= htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) ?></strong>, select <strong>Contact Tracing</strong>, select <strong><?= htmlspecialchars($location['Name']) ?></strong> and follow the on screen instructions
    </p>

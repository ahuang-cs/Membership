...
$username = "";
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-SE-ID'])) {
  $username = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-SE-ID'];
}
...
<div class="container">
  ...
  <div class="row justify-content-between">
    <div class="col-md-8 col-lg-5">
      ...
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-MemberLoginError'])) { ?>
        <div class="alert alert-danger">
          <p class="mb-0">
            <strong>We could not log you in</strong>
          </p>
          <p class="mb-0">
            <?= htmlspecialchars($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-MemberLoginError']) ?>
          </p>
        </div>
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-MemberLoginError']);
      } ?>
      ...
      <form method="post" action="<?= autoUrl("log-books/login") ?>" name="loginform" id="loginform" class="needs-validation" novalidate>
        ...
        <input type="hidden" name="target" value="<?= htmlspecialchars($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TARGET_URL']) ?>">
        <?= SCDS\CSRF::write() ?>

...
try {
  ...
  try {
    ...
    if ($getCount->fetchColumn() > 1) {
      throw new Exception('Your Swim England number is not unique in the ' . nezamy_app()->tenant->getKey('CLUB_NAME') . ' database. We cannot log you in with ambiguous details.');
    } 
    ...
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-MemberLoggedIn'] = true;
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-Member'] = $member['MemberID'];

    if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TARGET_URL'])) {
      unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TARGET_URL']);
    }
  ...
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-SE-ID'] = $_POST['swim-england'];
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-MemberLoginError'] = $e->getMessage();

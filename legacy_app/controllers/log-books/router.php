...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LoggedIn']) && bool($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LoggedIn'])) {
  ...
  } else if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] != 'Galas') {
    // Access for others
  ...
  if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-MemberLoggedIn']) && bool($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-MemberLoggedIn'])) {

    $this->get('/', function() {
      $member = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-Member'];
      include 'member-logs.php';
    });
    ...
    $this->group('/members/{member}:int/new', function($member) {
      $this->get('/', function($member) {
        if ($member == $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-Member']) {
          include 'new-log.php';
        } else {
          halt(404);
        }

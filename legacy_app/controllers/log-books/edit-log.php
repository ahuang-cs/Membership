...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Parent' && $info['UserID'] != $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']) {
  halt(404);
}

if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-MemberLoggedIn']) && bool($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-MemberLoggedIn']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LogBooks-Member'] != $info['MemberID']) {
  halt(404);
}
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-8">
        ...
        <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EditLogErrorMessage'])) { ?>
        <div class="alert alert-danger">
        <?=$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EditLogErrorMessage']?>
        </div>
        <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EditLogErrorMessage']); } ?>

        <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EditLogSuccessMessage']) && bool($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EditLogSuccessMessage'])) { ?>
        <div class="alert alert-success">
          <p class="mb-0">
            <strong>Changes saved successfully</strong>
          </p>
        </div>
        <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EditLogSuccessMessage']); } ?>

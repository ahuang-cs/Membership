...
if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationMode'] == "Family-Manual") {
  ...
  if (!$row) {
  	$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationFamNum'] = htmlentities($fid);
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationFamKey'] = htmlentities($acs);
  }

  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['FamilyIdentifier'] = $fid;
}
...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['FamilyIdentifier'])) {
  $account["FamilyIdentifier"] = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['FamilyIdentifier'];
...
if ($status) {
  ...
  <p>Thanks for signing up for your ' . nezamy_app()->tenant->getKey('CLUB_NAME') . ' Account.</p>
  ...
  notifySend($to, $subject, $sContent, $forename . " " . $surname, $email, ["Email" => "registration@" . getenv('EMAIL_DOMAIN'), "Name" => nezamy_app()->tenant->getKey('CLUB_NAME')]);

  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationGoVerify'] = '
  ...
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationUsername'] = $username;
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationForename'] = $forename;
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationSurname'] = $surname;
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationEmail'] = $email;
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationMobile'] = $mobile;
  if ($emailAuth == 1) {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationEmailAuth'] = " checked ";
  }
  if ($smsAuth == 1) {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegistrationSmsAuth'] = " checked ";
  }

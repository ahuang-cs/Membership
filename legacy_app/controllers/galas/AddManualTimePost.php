...
try {
	...
	if (bool($row['EntryProcessed']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Parent') {
	...
	$db->rollBack();
	$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateSuccess'] = false;
} catch (Exception $e) {
	$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateError'] = $e->getMessage();
}

...
$locked = "";
$processed = false;
if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Parent' && bool($row['EntryProcessed'])) {
	$locked = " disabled ";
	$processed = true;
}
...
if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Parent' && $row['UserID'] != $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']) {
	halt(404);
}
	...
	<div class="row">
		<div class="col-lg-8">
			...
			<div class="alert alert-warning">
				...
				<?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateError'])) { ?>
				<p class="mb-0 mt-3">
					<?=htmlspecialchars($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateError'])?>
				</p>
			...
			<?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateSuccess'])) {
				if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateSuccess']) { ?>
				...
				<?php }
				unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateSuccess']);
			} ?>

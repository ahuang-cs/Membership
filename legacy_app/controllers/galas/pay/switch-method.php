...
if ($_POST['method'] == 'select') {
  ...
  if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AddNewCard'])) {
  ...
  $getCards->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], $_POST['method']]);
  ...
  if ($details['count'] > 0) {
    try {
      ...
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentMethodID'] = $_POST['method'];
      $toId = '#saved-cards';
    } catch (Exception $e) {
      pre($e);
      halt(500);

...
$selected = null;
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentMethodID'])) {
  $selected = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentMethodID'];

  foreach ($cards as $card) {
    if ($card['ID'] == $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentMethodID']) {
...
if (!isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PaidEntries'])) {
  halt(404);
}
...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentIntent'])) {
  $intent = \Stripe\PaymentIntent::retrieve(
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentIntent'],
    [
      'stripe_account' => $tenant->getStripeAccount()
    ]
  );

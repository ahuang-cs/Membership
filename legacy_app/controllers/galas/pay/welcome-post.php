...
$entries->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], $date->format("Y-m-d")]);
} catch (Exception $e) {
  pre($e);
}
...
$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PaidEntries'] = $payingEntries;
...
if (sizeof($payingEntries) > 0) {
  ...
  if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentIntent']) && \Stripe\PaymentIntent::retrieve($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentIntent'], ['stripe_account' => $tenant->getStripeAccount()])->status != 'succeeded') {
    ...
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GalaPaymentIntent'] = $intent->id;

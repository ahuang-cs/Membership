...
$locked = false;
if (bool($row['Charged']) || bool($row['EntryProcessed']) || ($closingDate < $theDate && ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] != 'Admin' && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] != 'Galas')) || bool($row['Locked'])) {
  $locked = true;
}
...
<div class="container">
  <nav aria-label="breadcrumb">
    ...
    <div>
      ...
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateError']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateError']) { ?>
      <div class="alert alert-danger">A database error occured which prevented us saving the changes.</div>
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateError']); } ?>

      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateSuccess']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateSuccess']) { ?>
      <div class="alert alert-success">All changes to your gala entry have been saved.</div>
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UpdateSuccess']); } ?>
      ...
      <?php if ($row['StripePayment'] != null && bool($row['Paid'])) {
        ...
        <h2>Payment</h2>
        <p class="lead"><?php if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Parent') { ?>You<?php } else { ?><?=htmlspecialchars($parentName)?><?php } ?> paid for this gala entry by card</p>
        <div class="row align-items-center mb-3">
          <div class="col-auto">
            <img src="<?=url("/img/stripe/" . $row['Brand'] . ".png")?>" srcset="<?=url("/img/stripe/" . $row['Brand'] . "@2x.png")?> 2x, <?=url("/img/stripe/" . $row['Brand'] . "@3x.png")?> 3x" style="width:40px;"> <span class="sr-only"><?=htmlspecialchars(getCardBrand($row['Brand']))?></span>
          </div>
          <div class="col-auto">
            <p class="my-0">
        ...
        <?php if ($countPaid > 1) { ?>
        <p>
        <?php if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Parent') { ?>You<?php } else { ?><?=htmlspecialchars($parentName)?><?php } ?> paid for <?=htmlspecialchars($numFormat->format($countPaid))?> gala entries as part of this transaction.
        </p>
        <?php } ?>
        <p>

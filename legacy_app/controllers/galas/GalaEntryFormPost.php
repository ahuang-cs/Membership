...
if (isset($_POST['is-select-sessions']) && bool($_POST['is-select-sessions'])) {
  ...
  if ($swimmer == null || ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Parent' && $swimmer != $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'])) {
    halt(404);
  }
  ...
  try {
    ...
    if ($getGalaEntries->fetchColumn() > 0) {
      ...
      $email->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], $subject, $message]);

      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SuccessfulGalaEntry'] = [
        "Gala" => $_POST['gala'],

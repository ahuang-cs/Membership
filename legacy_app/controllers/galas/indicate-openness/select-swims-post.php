...
try {
  ...
  foreach ($users as $user => $entries) {
    ...
    $message = '<p>' . htmlspecialchars($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Forename'] . ' ' . $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Surname']) . ' has entered ';
  ...
  $db->commit();
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SuccessStatus'] = true;
} catch (Exception $e) {
  $db->rollBack();
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorStatus'] = true;
}

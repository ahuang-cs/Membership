...
<div class="container-fluid">
  ...
  <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['User-Update-Email-Error']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['User-Update-Email-Error']) { ?>
    <div class="alert alert-danger">
      <strong>We were not able to update the user's email address because it was not valid</strong>
    </div>
  <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['User-Update-Email-Error']);
  } ?>

  <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['User-Update-Email-Success']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['User-Update-Email-Success']) { ?>
    <div class="alert alert-success">
      <strong>We've updated the user's email address</strong>
    </div>
  <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['User-Update-Email-Success']);
  } ?>

  <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivSuccess']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivSuccess']) { ?>
    <div class="alert alert-success">
      <strong>We've sent your email to <?= htmlspecialchars($info['Forename']) ?></strong>
    </div>
  <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['NotifyIndivSuccess']);
  } ?>

  <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserCreationSuccess']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserCreationSuccess']) { ?>
    <div class="alert alert-success">
      <p class="mb-0">
  ...
  <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserCreationSuccess']);
  } ?>
    ...
    <div class="col-md-4 col-lg-3 col-xl-3">
      <div class="position-sticky top-3 card mb-3">
        ...
        <div class="list-group list-group-flush" id="member-page-menu">
          ...
          <?php } ?>
          <?php if ($userObj->hasPermission('Coach') && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Admin') { ?>
          ...
          <?php if ($userObj->hasPermission('Parent')) { ?>
            ...
            <?php if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Admin' && (nezamy_app()->tenant->getGoCardlessAccessToken()) && !userHasMandates($id)) { ?>

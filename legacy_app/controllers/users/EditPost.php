...
try {
  ...
  if (!v::email()->validate($email)) {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['InvalidEmail'] = true;
    throw new Exception();
  }
  ...
  if ($get->fetchColumn() > 0) {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UsedEmail'] = true;
    throw new Exception();
  }
  ...
  try {
    ...
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['InvalidPhone'] = true;
    throw new Exception();
  }
  ...
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Success'] = true;
} catch (Exception $e) {
  // reportError($e);
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GeneralError'] = true;
}

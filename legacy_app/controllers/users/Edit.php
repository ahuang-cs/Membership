...
<div class="container">
  ...
  <div class="row">
    <div class="col-lg-8">
      ...
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Success']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Success']) { ?>
        <div class="alert alert-success">
          <p class="mb-0">User details updated.</p>
        </div>
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Success']);
      } ?>

      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['InvalidEmail']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['InvalidEmail']) { ?>
        <div class="alert alert-danger">
          <p class="mb-0">The email you entered is invalid.</p>
        </div>
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['InvalidEmail']);
      } ?>

      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UsedEmail']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UsedEmail']) { ?>
        <div class="alert alert-danger">
          <p class="mb-0">The email you entered is already in use by another account.</p>
        </div>
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UsedEmail']);
      } ?>

      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['InvalidPhone']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['InvalidPhone']) { ?>
        <div class="alert alert-danger">
          <p class="mb-0">The phone number you entered is invalid.</p>
        </div>
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['InvalidPhone']);
      } ?>

      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GeneralError']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GeneralError']) { ?>
        <div class="alert alert-danger">
          <p class="mb-0">An unspecified error occurred. Please try again.</p>
        </div>
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['GeneralError']);
      } ?>

...
try {
  ...
  if (!\SCDS\FormIdempotency::verify() || !\SCDS\CSRF::verify()) {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorInvalidRequest'] = true;
  } else {
    try {
      // Get renewal
      $db = nezamy_app()->db;
      ...
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Successful'] = true;
    } catch (Exception $e) {
      // Catches halt
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorNoReg'] = true;
    }
  }
} catch (Exception $e) {

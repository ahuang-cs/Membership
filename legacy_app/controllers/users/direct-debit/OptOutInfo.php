    ...
    <div class="col-lg-8">
      <h1>Allow a Direct Debit Opt Out</h1>
        <form method="post">
          ...
          <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorInvalidRequest']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorInvalidRequest']) { ?>
          <div class="alert alert-warning">
            <strong>We couldn't verify you had the authority to do that</strong>
          </div>
          <?php } unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorInvalidRequest']); ?>

          <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorNoReg']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorNoReg']) { ?>
          <div class="alert alert-warning">
            <strong>There was no open registration or renewal</strong>
          </div>
          <?php } unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorNoReg']); ?>

          <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Successful']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Successful']) { ?>
          <div class="alert alert-success">
            <strong>We've marked the renewal/registration as complete and bypassed the direct debit requirement</strong>
          </div>
          <?php } unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['Successful']); ?>

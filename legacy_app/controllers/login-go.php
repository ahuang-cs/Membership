    ...
    if (password_verify($_POST['password'], $hash)) {
      ...
      if ($isNotJustParent || bool(getUserOption($userID, "Is2FA")) || $do_random_2FA) {
        ...
        if (bool(getUserOption($userID, "hasGoogleAuth2FA"))) {
          $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR_GOOGLE'] = true;
        } else {
          ...
          if (notifySend(null, "Verification Code - Requested at " . $date->format("H:i:s \o\\n d/m/Y"), $message, $forename . " " . $surname, $email)) {
            $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['TWO_FACTOR_CODE'] = $code;
          } else {
            halt(500);
          }
        }
        $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['2FAUserID'] = $userID;
        if ($_POST['RememberMe']) {
          $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['2FAUserRememberMe'] = 1;
        }
      ...
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EnteredUsername'] = $username;
    }
  } else {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorState'] = true;
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EnteredUsername'] = $username;

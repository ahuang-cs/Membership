...
try {
  foreach ($vars as $key => $value) {
    if (isset($_POST[$key]) && $_POST[$key] != null && !getenv($key)) {
      nezamy_app()->tenant->setKey($key, $_POST[$key]);
    }
  }

  if (isset($_POST['CLUB_INFO']) && nezamy_app()->tenant->getKey('ASA_CLUB_CODE') != $_POST['CLUB_INFO'] && $_POST['CLUB_INFO'] != '0000') {
    nezamy_app()->tenant->setKey('CLUB_NAME', $clubs[$_POST['CLUB_INFO']]['Name']);
    nezamy_app()->tenant->setKey('CLUB_SHORT_NAME', $clubs[$_POST['CLUB_INFO']]['MeetName']);
    nezamy_app()->tenant->setKey('ASA_CLUB_CODE', $clubs[$_POST['CLUB_INFO']]['Code']);
    nezamy_app()->tenant->setKey('ASA_DISTRICT', $clubs[$_POST['CLUB_INFO']]['District']);
    nezamy_app()->tenant->setKey('ASA_COUNTY', $clubs[$_POST['CLUB_INFO']]['County']);
  }
  ...
  nezamy_app()->tenant->setKey('EMERGENCY_MESSAGE', $message);
  ...
  if ($type == 'NONE' || $type == 'SUCCESS' || $type == 'WARN' || $type == 'DANGER') {
    ...
    nezamy_app()->tenant->setKey('EMERGENCY_MESSAGE_TYPE', $type);
  }
  ...
  nezamy_app()->tenant->setKey('HIDE_MEMBER_ATTENDANCE', $hide);

  if (nezamy_app()->tenant->getKey('LOGO_DIR')) {
    nezamy_app()->tenant->setKey('SHOW_LOGO', (int) bool($_POST['SHOW_LOGO']));
  }
  ...
  nezamy_app()->tenant->setKey('HIDE_CONTACT_TRACING_FROM_PARENTS', $hide);
  ...
  nezamy_app()->tenant->setKey('BLOCK_SQUAD_REPS_FROM_NOTIFY', $hide);
  ...
  nezamy_app()->tenant->setKey('REQUIRE_FULL_REGISTRATION', $hide);
  ...
  nezamy_app()->tenant->setKey('REQUIRE_FULL_RENEWAL', $hide);
  ...
  nezamy_app()->tenant->setKey('ENABLE_BILLING_SYSTEM', $hide);
  ...
  nezamy_app()->tenant->setKey('CLUB_ADDRESS', $addr);

...
foreach ($vars as $key => $value) {
  if (getenv($key)) {
    ...
    $vars[$key] = nezamy_app()->tenant->getKey($key);
    $disabled[$key] = '';
  }
}
    ...
    <div class="col-md-9">
      <main>
        ...
        <form method="post" class="needs-validation" novalidate>
          ...
          <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PCC-SAVED']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PCC-SAVED']) { ?>
            <div class="alert alert-success">System variables saved.</div>
          <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PCC-SAVED']);
          } ?>

          <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PCC-ERROR']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PCC-ERROR']) { ?>
            <div class="alert alert-danger">Changes were not saved.</div>
          <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['PCC-ERROR']);
          } ?>
          ...
          <div class="row">
            ...
            <div class="col col-lg-4">
              <img src="<?= htmlspecialchars(url("/img/settings/emergency-message.png")) ?>" class="border" alt="Picture of membership software displaying emergency message">
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <select class="custom-select" name="CLUB_INFO" id="CLUB_INFO">
              <?php foreach ($clubs as $club) { ?>
                <option <?php if ($club['Code'] == nezamy_app()->tenant->getKey('ASA_CLUB_CODE')) { ?>selected<?php } ?> value="<?= htmlspecialchars($club['Code']) ?>">

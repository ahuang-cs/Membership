...
$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-FC'] = $_POST;
...
if (!v::email()->validate($_POST['email-addr'])) {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors']['Email'] = "The email address is invalid";
...
if (!v::date()->validate($dob)) {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors']['DOB'] = "The date of birth provided is not valid";
  header("Location: " . autoUrl("services/request-a-trial"));
}

if ($_POST['forename'] == "") {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors']['Parent-FN'] = "No parent first name";
  header("Location: " . autoUrl("services/request-a-trial"));
}

if ($_POST['surname'] == "") {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors']['Parent-LN'] = "No parent last name";
  header("Location: " . autoUrl("services/request-a-trial"));
}

if (!$isParent) {
  if ($_POST['swimmer-forename'] == "") {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors']['Swimmer-FN'] = "No swimmer first name";
    header("Location: " . autoUrl("services/request-a-trial"));
  }

  if ($_POST['swimmer-surname'] == "") {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors']['Swimmer-LN'] = "No swimmer last name";
    header("Location: " . autoUrl("services/request-a-trial"));
  }
}

if ($_POST['sex'] == "") {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors']['Swimmer-LN'] = "No swimmer sex provided";
  header("Location: " . autoUrl("services/request-a-trial"));
}

if ($_POST['experience'] == "") {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors']['Swimmer-Experience'] = "No experience option selected";
  header("Location: " . autoUrl("services/request-a-trial"));
}

/*
if (true) {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors']['TESTING'] = "Testing system";
  header("Location: " . autoUrl("services/request-a-trial"));
}
*/

if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors'])) {
  die();
}
...
$email_parent = '
<p>Hello ' . $parent . '</p>
<p>Thanks for your interest in a trial ' . $forText . ' at ' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME')) . '. We\'re working through your request and will get back to you as soon as we can.</p>';

$to_club = notifySend(null, 'New Trial Request', $email_club, 'Club Admins', htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_TRIAL_EMAIL')), ["Email" => "join@" . getenv('EMAIL_DOMAIN'), "Name" => nezamy_app()->tenant->getKey('CLUB_NAME'), 'Reply-To' => $_POST['email-addr']]);
...
if ($to_club && $to_parent) {
  ...
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Success'] = true;
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-AddAnother'] = [
    'forename' => $_POST['forename'],
    'surname' => $_POST['surname'],
    'email-addr' => $_POST['email-addr']
  ];
} else {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Success'] = false;
}

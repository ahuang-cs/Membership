...
$noValues = true;
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-FC'])) {
  $value = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-FC'];
  $noValues = false;
}

if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-AddAnother'])) {
  $value = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-AddAnother'];
  $noValues = false;
}
...
<div class="container">
  ...
  <div class="row">
    <div class="col-md-10 col-lg-8">
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Success']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Success'] === false) { ?>
      ...
      <?php } ?>
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors'])) { ?>
        <div class="alert alert-danger">
          There was a problem with some of the information you supplied. Please check;
          <ul class="mb-0">
          <?php foreach ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors'] as $error) { ?>
            <li><?=$error?></li>
          <?php } ?>
          </ul>
        </div>
      <?php } ?>
      <p class="lead">
        It's great that you want to request a trial at <?=htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))?>. To help us
      ...
      <form method="post" <?php if (!$noValues) { ?>class="was-validated"<?php } else { ?>class="needs-validation" novalidate<?php } ?>>
        <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Admin') { ?>
        <div id="begin">
        ...
        <div id="swimmers">
          ...
          <div class="mb-3">
            ...
            <input type="test" name="swimmer-club" id="swimmer-club" class="form-control" placeholder="<?=htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))?>" value="<?=htmlspecialchars(trim($value['swimmer-club']))?>">
          </div>
          <div class="mb-3">
        ...
        <p>
          By submitting this form you agree to let <?=htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))?> contact you by
        ...
        <p>
          <?=htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))?> will not use your email address for marketing purposes.
        </p>
...
unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-FC']);
unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequestTrial-Errors']);

...
<div class="container">
  ...
  <form method="post" id="notify-form" onkeypress="return event.keyCode != 13;" enctype="multipart/form-data" novalidate>
    ...
    <div class="form-group">
      ...
      <textarea class="form-control" id="message" name="message" rows="10" data-tinymce-css-location="<?= htmlspecialchars(url("/css/tinymce.css")) ?>" data-documentBaseUrl="<?= htmlspecialchars(autoUrl("notify/new/")) ?>" required><?php if (isset($_SESSION['SCDS-Notify']['NotifyPostData']['message'])) { ?><?= htmlspecialchars($_SESSION['SCDS-Notify']['NotifyPostData']['message']) ?><?php } ?></textarea>
    </div>

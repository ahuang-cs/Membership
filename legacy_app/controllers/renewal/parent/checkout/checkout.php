...
try {
  $res = \Stripe\ApplePayDomain::create([
    'domain_name' => nezamy_app('request')->hostname
  ], [
...
$customer->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
...
$numberOfCards->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], 1, $expYear, $expYear, $expMonth]);
...
$selected = null;
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalPaymentMethodID'])) {
  $selected = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalPaymentMethodID'];

  foreach ($cards as $card) {
    if ($card['ID'] == $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalPaymentMethodID']) {
...
if (!isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalPaymentIntent'])) {
  halt(404);
}
...
if (!isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalPaymentMethodID'])) {
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AddNewCard'] = true;
}
...
$renewal = $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalID'];

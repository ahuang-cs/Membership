...
if ($_POST['agree'] == 1) {
	...
	if (isPartialRegistration()) {
		$nextMember = getNextSwimmer($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], $id, true);
	} else {
		$nextMember = getNextSwimmer($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], $id);
	}
	...
	$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RenewalErrorInfo'] = '

...
try {
	...
	$discounts = json_decode(nezamy_app()->tenant->getKey('MembershipDiscounts'), true);
	...
	$sql->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
	...
	$getMembers->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
	...
	$asa1 = nezamy_app()->tenant->getKey('ASA-County-Fee-L1') + nezamy_app()->tenant->getKey('ASA-Regional-Fee-L1') + nezamy_app()->tenant->getKey('ASA-National-Fee-L1');
	$asa2 = nezamy_app()->tenant->getKey('ASA-County-Fee-L2') + nezamy_app()->tenant->getKey('ASA-Regional-Fee-L2') + nezamy_app()->tenant->getKey('ASA-National-Fee-L2');
	$asa3 = nezamy_app()->tenant->getKey('ASA-County-Fee-L3') + nezamy_app()->tenant->getKey('ASA-Regional-Fee-L3') + nezamy_app()->tenant->getKey('ASA-National-Fee-L3');
	...
	if (
		...
		$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalID'] = $renewal;
		...
		if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalPaymentIntent']) && \Stripe\PaymentIntent::retrieve($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalPaymentIntent'], ['stripe_account' => $tenant->getStripeAccount()])->status != 'succeeded') {
			$intent = \Stripe\PaymentIntent::retrieve(
				$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalPaymentIntent'],
				[
			...
			$intent = \Stripe\PaymentIntent::create([
				...
				'metadata' => [
					...
					'renewal_id' => $renewal,
					'user_id' => $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'],
				]
			], [
				'stripe_account' => $tenant->getStripeAccount()
			]);
			$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalPaymentIntent'] = $intent->id;

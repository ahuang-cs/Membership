...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalDDSuccess'])) {
  ...
  unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalDDSuccess']);
  ...
  if ($hasStripeMandate || $hasGCMandate || isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RegRenewalDDSuccess'])) {
    $nextSubstage = $db->prepare("UPDATE `renewalProgress` SET `Stage` = `Stage` + 1, `Substage` = 0 WHERE `RenewalID` = ? AND `UserID` = ?");
    $nextSubstage->execute([
      $renewal,
      $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']
    ...
    <div class="row">
      <div class="col-lg-8">
        <form method="post">
          ...
          <?php } else if (nezamy_app()->tenant->getGoCardlessAccessToken()) { ?>
            <p>
              <button type="submit" class="btn btn-success btn-lg btn-block">
                Setup direct debit
              </button>
          ...
          <?php if (!$hasStripeMandate && nezamy_app()->tenant->getBooleanKey('ALLOW_DIRECT_DEBIT_OPT_OUT')) { ?>

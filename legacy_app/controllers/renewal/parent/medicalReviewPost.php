...
try {
  ...
  $medUpdate->execute([
	...
	if (isPartialRegistration()) {
		$nextSwimmer = getNextSwimmer($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], $id, true);
	} else {
		$nextSwimmer = getNextSwimmer($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], $id);
	}

	if ($nextSwimmer == null) {
		$nextSection = $db->prepare("UPDATE `renewalProgress` SET `Stage` = `Stage` + 1, `Substage` = '0',
		`Part` = '0' WHERE `RenewalID` = ? AND `UserID` = ?");
		$nextSection->execute([$renewal, $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
	} else {
		...
		$nextSection->execute([$nextSwimmer, $renewal, $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
	}

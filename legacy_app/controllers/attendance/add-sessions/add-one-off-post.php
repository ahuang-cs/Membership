...
if (!\SCDS\CSRF::verify()) {
  ...
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AddSessionError'] = 'We could not validate your CSRF token. Please try again.';
...
if ($error) {
  http_response_code(302);
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AddSessionError'] = $errors;
  header('location: ' . autoUrl('attendance/sessions/new'));
  return;
}

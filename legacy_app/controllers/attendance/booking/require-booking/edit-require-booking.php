  ...
  <div class="row">
    <div class="col-lg-8">
      <form class="needs-validation" method="post" action="<?= htmlspecialchars(autoUrl('sessions/booking/edit')) ?>" novalidate>
        ...
        <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequireBookingError'])) { ?>
          <div class="alert alert-warning">
            <p class="mb-0">
              <strong>Error</strong>
            </p>
            <p class="mb-0">
              <?= htmlspecialchars($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequireBookingError']) ?>
            </p>
          </div>
        <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['RequireBookingError']);
        } ?>

        <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EditRequireBookingSuccess'])) { ?>
          <div class="alert alert-success">
            <p class="mb-0">
              <strong>Changes saved successfully</strong>
            </p>
          </div>
        <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EditRequireBookingSuccess']);
        } ?>

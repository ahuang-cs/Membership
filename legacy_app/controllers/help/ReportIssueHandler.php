...
<div class="container">
	...
	<?php if (nezamy_app()->tenant && isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorReportStatus']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorReportStatus'] == true) { ?>
		<p>We have reported that page to our team.</p>
		<p>Thank you for your feedback. It really helps us improve our website.</p>
		<p>
			<a href="<?= htmlspecialchars($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorReportTarget']) ?>" class="btn btn-secondary">
				Return to Page
			</a>
		</p>
		<?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorReportTarget']); ?>
	<?php } else if (!isset($_GET['url']) || (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorReportStatus']) &&
		$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorReportStatus'] == false)) { ?>
	...
	<?php } else { ?>
		...
		<form method="post" class="needs-validation" novalidate>
			...
			<div class="form-group">
				...
				<input type="email" <?php if (isset(nezamy_app()->user)) { ?>value="<?= htmlspecialchars(nezamy_app()->user->getEmail()) ?>" readonly<?php } else { ?> <?php } ?> class="form-control" id="email-address" name="email-address" required>
...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorReportStatus'])) {

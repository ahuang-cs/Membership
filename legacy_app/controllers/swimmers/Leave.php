...
if ($result == null || $result != $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']) {
  halt(404);
}
...
$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LeaveKey'] = hash('md5', time());
...
<div class="container">
  <div class="row">
    <div class="col-lg-8">
      <?php if ($count == 0 && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ConfirmLeave'] !== true) { ?>
      <h1>
      ...
      <p>
        <a href="<?=autoUrl("members/" . $id . "/leaveclub/" . $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LeaveKey'])?>" class="btn btn-danger">
          I Confirm
        </a>
      </p>
      <?php } else if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ConfirmLeave']) { ?>
      <h1>
        We're sorry to see you go
      </h1>
      <p>
...
if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ConfirmLeave'])) {
  unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ConfirmLeave']);
}

...
if ((!empty($_POST['forename'])) && (!empty($_POST['surname'])) && (!empty($_POST['datebirth'])) && (!empty($_POST['sex']))) {
	...
	try {
		...
		if ($getASA) {
			$swimEnglandTemp = nezamy_app()->tenant->getKey('ASA_CLUB_CODE') . $last_id;
...
if ($action) {
	$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SwimmerAdded'] = true;

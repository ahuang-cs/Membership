...
/** must go in form
if (nezamy_app('request')->method == 'POST' && !SCDS\CSRF::verify()) {
	halt(403);
}
 */
...
} else if (isset($_POST['asa']) && $_POST['asa'] == "" && nezamy_app('request')->method == "post") {
	$newASANumber = nezamy_app()->tenant->getKey('ASA_CLUB_CODE') . $id;
	if ($newASANumber != $asaNumber) {
		$updateSwimmer = $db->prepare("UPDATE `members` SET `ASANumber` = ? WHERE `MemberID` = ?");
		$updateSwimmer->execute([$newASANumber, $id]);
		$asaUpdate = true;
...
if (isset($_POST['swimmerStatus']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == "Admin") {

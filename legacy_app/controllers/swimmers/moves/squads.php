...
try {
  ...
  if (!nezamy_app()->user->hasPermission('Admin') && !nezamy_app()->user->hasPermission('Coach') && (isset($member) && $member->getUser()->getId() != $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'])) {
    http_response_code(404);
    echo json_encode([
      'status' => 404,
      'message' => 'Not Found'

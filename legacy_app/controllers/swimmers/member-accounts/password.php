...
if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == 'Parent' && $member['uid'] != $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']) {
  halt(404);
}
...
<div class="container">
  ...
  <div class="row">
    <div class="col-lg-8">
      ...
      <?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SetMemberPassError'])) { ?>
      <div class="alert alert-danger">
        <p class="mb-0">
          <strong>We could not set the password.</strong>
        </p>
        <p class="mb-0">
          <?=htmlspecialchars($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SetMemberPassError'])?>
        </p>
      </div>
      <?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SetMemberPassError']); } ?>
      ...
      <p>Usual <?=htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_NAME'))?> password policies apply - Use 8 characters or more, with at least one lowercase letter, at least one uppercase letter and at least one number</p>

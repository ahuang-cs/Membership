...
try {
  ...
  if ($adminMode) {
    ...
    $asaNumber = mb_strtoupper(nezamy_app()->tenant->getKey('ASA_CLUB_CODE')) . $id;
  ...
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['SuccessState'] = true;
} catch (Exception $e) {
  $db->rollBack();

  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ErrorState'] = $e->getMessage();
}

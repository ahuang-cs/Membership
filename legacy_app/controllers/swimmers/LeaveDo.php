...
if ($row == null || $row['UserID'] != $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']) {
  halt(404);
}
...
if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LeaveKey'] == $key) {
  try {
    ...
    $subject = $row['MForename'] . ' ' . $row['MSurname'] . ' is leaving ' . nezamy_app()->tenant->getKey('CLUB_NAME');
    ...
    unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['LeaveKey']);
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['ConfirmLeave'] = true;

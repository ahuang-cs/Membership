...
if (trim($_POST['email-addr']) != $parent['Email'] && v::email()->validate($_POST['email-addr'])) {
  ...
  // cannot send email
  $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailInvalid'] = true;
}
...
notifySend(null, 'Join ' . nezamy_app()->tenant->getKey('CLUB_NAME'), $email, $parent['First'] . ' ' . $parent['Last'], $_POST['email-addr']);

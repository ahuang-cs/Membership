...
if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == "Parent" && $parent != $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']) {
  halt(404);
}

$parent_name = getUserName($parent);
$downloader_name = getUserName($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']);
...
fputcsv($output, array(nezamy_app()->tenant->getKey('CLUB_NAME') . ' Member Data Download'));
...
fputcsv($output, array(nezamy_app()->tenant->getKey('CLUB_NAME') . ' GDPR Compliance Team', 'via Membership Software', 'by SCDS'));

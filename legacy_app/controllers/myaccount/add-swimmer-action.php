...
if (mb_stripos($asaNumber, nezamy_app()->tenant->getKey('ASA_CLUB_CODE'))) {
  $asaNumber = str_replace(nezamy_app()->tenant->getKey('ASA_CLUB_CODE'), '', $asaNumber);
}
...
if ($asaNumber != null && $accessKey != null && v::alnum()->validate($asaNumber) && v::alnum()->validate($accessKey)) {
  if ($row != null) {
    ...
    if ($row['UserID'] != null) {
      ...
      from your account.</p>
      <p>If this was not you, contact <a href=\"mailto:" . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_EMAIL')) . "\">" . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_EMAIL')) . "</a> as soon as possible</p>";
    ...
    $updateSwimmer->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], $accessKey, $memberID]);
    ...
    <ul>
      ...
      <li>" . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_SHORT_NAME')) . " Member ID: " . htmlspecialchars($row['MemberID']) . "</li>
    </ul>
    <p>If this was not you, contact <a href=\"mailto:"  . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_EMAIL')) . "\">
    "  . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_EMAIL')) . "</a> as soon as possible</p>";
    notifySend($row['EmailAddress'], "You've added " . $row['MForename'] . " to your account",
    $message, $row['Forename'] . " " . $row['Surname'],
    $row['EmailAddress']);

    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AddSwimmerSuccessState'] = "

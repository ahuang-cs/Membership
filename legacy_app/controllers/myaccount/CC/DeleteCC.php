...
try {
  ...
  $count->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], $id]);
  $before = $count->fetchColumn();

  $delete = $db->prepare("DELETE FROM notifyAdditionalEmails WHERE UserID = ? AND ID = ?");
  $delete->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], $id]);
  ...
  if ($after < $before) {
    $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['DeleteCCSuccess'] = true;
  }
} catch (Exception $e) {
  $db->rollBack();
}

...
if ($email_comms != $row['EmailComms']) {
  ...
  try {
  	$db->prepare($sql)->execute([$emailCommsDb, $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
  } catch (Exception $e) {
		// Could not update settings
		$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdateError'] = '<p class="mb-0"><strong>We were unable to change your email subscription preferences</strong></p><p class="mb-0">Please try again. If the issue persists, please contact support referencing <span class="mono">Email Preferences Update Error</span></p>';
  }
}
	...
	if (v::email()->validate(mb_strtolower($_POST['EmailAddress']))) {
	  ...
	  $sql = 'INSERT INTO `newUsers` (`AuthCode`, `UserJSON`, `Type`) VALUES (?, ?, ?)';
		try {
			...
			// Could not add to db
			reportError($e);
			$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdateError'] = '<p class="mb-0"><strong>We were unable to add your new email address to our awaiting confirmation list</strong></p><p class="mb-0">Please try again. If the issue persists, please contact support referencing <span class="mono">Email Address Update Error</span></p>';
		}
		$id = $db->lastInsertId();
	  ...
	  href="mailto:' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_EMAIL')) . '">' . htmlspecialchars(nezamy_app()->tenant->getKey('CLUB_EMAIL')) . '</a>/</p>
	  ';
	  notifySend($to, $subject, $sContent, $name, mb_strtolower($_POST['EmailAddress']), ["Email" => "support@" . getenv('EMAIL_DOMAIN'), "Name" => nezamy_app()->tenant->getKey('CLUB_NAME') . " Security"]);
		$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdate'] = true;
		$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdateNew'] = mb_strtolower($_POST['EmailAddress']);
	} else {
		$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdate'] = false;
	}
}

...
try {
  ...
  $getExtraEmails->execute([$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID']]);
} catch (Exception $e) {}
...
if (isSubscribed($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], 'Security')) {
	$emailChecked_security = " checked ";
}

$emailChecked_payments;
if (isSubscribed($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], 'Payments')) {
	$emailChecked_payments = " checked ";
}

$emailChecked_new_member;
if ($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['AccessLevel'] == "Admin" && isSubscribed($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['UserID'], 'NewMember')) {
  ...
  <div class="row justify-content-between">
    ...
    <div class="col-md-9">
      ...
      <p class="lead">Manage your email address and email options.</p>
    	...
    	<?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['OptionsUpdate']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['OptionsUpdate']) { ?>
    		<div class="alert alert-success">
    			<p class="mb-0">
    				<strong>We've successfully updated your email options</strong>
    			</p>
    		</div>
    	<?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['OptionsUpdate']);
    	} ?>

			<?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdateError'])) { ?>
    		<div class="alert alert-success">
    			<p class="mb-0">
    				<?=$_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdateError']?>
    			</p>
    		</div>
    	<?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdateError']);
    	} ?>

    	<?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdate']) && $_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdate']) { ?>
    		<div class="alert alert-success">
    			<p class="mb-0">
    	...
    	<?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdate']);
    	} else if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdate'])) { ?>
    	...
    	<?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdateNew'])) { ?>
    		<div class="alert alert-info">
    			<p class="mb-0">
    				<strong>Once verified, your account email
    				address will change to
    				<?=htmlentities($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdateNew'])?></strong>
    			</p>
    		</div>
    	<?php } ?>
    	...
    	<div class="cell">
    		<form method="post">
    		    ...
    		    <input type="email" class="form-control" id="EmailAddress" name="EmailAddress" placeholder="name@example.com" value="<?=htmlentities($email)?>">
    				<?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdateNew'])) { ?>
    				<small class="form-text">Once verified, your account email address will change to <?=htmlentities($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['EmailUpdateNew'])?></small>
    				<?php } ?>
    		  </div>
      ...
      <div class="cell">
        ...
        <form id="cc" method="post" action="<?=autoUrl("my-account/email/cc/new")?>" class="needs-validation" novalidate>
					...
					<?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['VerifyEmailError']) && bool($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['VerifyEmailError'])) { ?>
					<div class="alert alert-warning">
						<p class="mb-0"><strong>There was a problem with the information you supplied.</strong></p>
						<p class="mb-0">Please try again.</p>
					</div>
					<?php unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['VerifyEmailError']); } ?>

					<?php if (isset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['DeleteCCSuccess'])) {
						unset($_SESSION['TENANT-' . nezamy_app()->tenant->getId()]['DeleteCCSuccess']); ?>
						<div class="alert alert-success">
							<p class="mb-0">

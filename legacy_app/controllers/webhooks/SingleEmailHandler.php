...
if (!nezamy_app()->tenant->isCLS()) {
	$emailPrefix = mb_strtolower(trim(nezamy_app()->tenant->getKey('ASA_CLUB_CODE'))) . '-';
}
	...
	if ((isSubscribed($row['UserID'], $row['EmailType']) || $row['ForceSend'] == 1) && bool($row['Active'])) {
		...
		if ($row['EmailType'] == 'Payments') {
			...
			$from = [
				...
				"Name" => nezamy_app()->tenant->getKey('CLUB_NAME') . " Galas"
			];
		} else if ($row['EmailType'] == 'Security') {
			$from = [
				"Email" => $emailPrefix . "security-support@" . getenv('EMAIL_DOMAIN'),
				"Name" => nezamy_app()->tenant->getKey('CLUB_NAME') . " Security",
				"Unsub" => [
					"Allowed" => true,
					"User" => $row['UserID'],
					"List" =>	"Security"
			...
			$from = [
				...
				"Name" => nezamy_app()->tenant->getKey('CLUB_SHORT_NAME') . " API Alerts",
				"Unsub" => [
					"Allowed" => true,
					"User" => $row['UserID'],
					"List" =>	"NewMember"
			...
			$from = [
				...
				"Name" => nezamy_app()->tenant->getKey('CLUB_SHORT_NAME') . " Staff"
			];
		}
